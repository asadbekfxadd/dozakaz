...
def create_order():
    ...
    try:
        ...
        msg = f'{prio_icon} <b>Новая заявка #{order_id}</b>\n🏪 Филиал: <b>{branch}</b>\n👤 {responsible}\n📅 {date}'
        if note: msg += f'\n💬 {note}'
        if priority == 'Срочно': msg += '\n\n🚨 <b>СРОЧНО!</b>'
        # Notify admins
        notify_role('admin', msg)
        notify_role('regional', msg)
        # Notify branch manager - confirm receipt
        notify_role('branch', f'✅ <b>Заявка #{order_id} принята!</b>\n🏪 {branch}\n📅 {date}\nСтатус: <b>Новая</b>\n\nМы получили вашу заявку и начнём обработку.', branch=branch)
    except Exception as e:
        print(f'[TG ORDER] {e}')
...
def update_status(oid):
    ...
    row = cur.fetchone()
    cur.close(); conn.close()
    # TG notification on status change
    try:
        if row:
            branch = row['branch']
            STATUS_ICONS = {'В работе': '🔄', 'Выполнена': '✅', 'Новая': '📋'}
            icon = STATUS_ICONS.get(status, '📋')
            msg = f'{icon} <b>Заявка #{oid} — {status}</b>\n🏪 {branch}\n📅 {row["date"]}'
            if status == 'В работе':
                msg += '\n\nВаша заявка взята в работу!'
                notify_role('branch', msg, branch=branch)
            elif status == 'Выполнена':
                msg += '\n\n🎉 Ваша заявка выполнена!'
                notify_role('branch', msg, branch=branch)
                notify_role('admin', f'✅ Заявка #{oid} ({branch}) выполнена')
    except Exception as e:
        print(f'[TG STATUS] {e}')
...
def tg_webhook():
    ...
    BRANCHES_LIST = [
        ...
        'UZBEGIM ANDIJAN','Yunusabad gallery'
    ]
    ...
    def save_sub(chat_id, username, full_name, role):
        ...
        cur2.execute('INSERT INTO tg_subscribers (chat_id,username,full_name,role,active) VALUES (%s,%s,%s,%s,TRUE) ON CONFLICT (chat_id) DO UPDATE SET role=%s,username=%s,full_name=%s,active=TRUE',
            (chat_id, username, full_name, role, role, username, full_name))
        c2.commit(); cur2.close(); c2.close()

    def start_kb():
        return {'inline_keyboard': [
            [{'text': '👔 Администратор', 'callback_data': 'role:admin'}],
            [{'text': '📦 Склад', 'callback_data': 'role:warehouse'}],
            [{'text': '🏪 Менеджер магазина ▶', 'callback_data': 'show_branches'}],
            [{'text': '📊 Региональный', 'callback_data': 'role:regional'}],
        ]}
    ...
    if 'callback_query' in data:
        ...
        if cb == 'show_branches':
            answer_cb(cq['id'], 'Выберите магазин')
            rows = [[{'text': f'🏪 {b}', 'callback_data': f'branch:{b}'}] for b in BRANCHES_LIST]
            rows.append([{'text': '◀ Назад', 'callback_data': 'back'}])
            send_msg(chat_id, '🏪 <b>Выберите ваш магазин:</b>', {'inline_keyboard': rows})

        elif cb == 'back':
            answer_cb(cq['id'])
            send_msg(chat_id, '🔔 <b>LI-NING — Уведомления</b>\n\nВыберите роль:', start_kb())

        elif cb.startswith('role:'):
            role = cb[5:]
            RNAMES = {'admin':'👔 Администратор','warehouse':'📦 Склад','regional':'📊 Региональный'}
            rname = RNAMES.get(role, role)
            save_sub(chat_id, username, full_name, role)
            answer_cb(cq['id'], 'Сохранено!')
            notif = {'admin':'• Новые заявки\n• Срочные заявки\n• Схлопки готовы',
                     'warehouse':'• Схлопка готова к отгрузке',
                     'regional':'• Заявки региона'}.get(role,'• Уведомления')
            send_msg(chat_id, f'✅ <b>Роль: {rname}</b>\n\nБудете получать:\n{notif}\n\n/start — сменить роль\n/stop — отключить\n/status — статус')

        elif cb.startswith('branch:'):
            branch = cb[7:]
            save_sub(chat_id, username, full_name, f'branch:{branch}')
            answer_cb(cq['id'], 'Магазин сохранён!')
            send_msg(chat_id, f'✅ <b>Магазин: {branch}</b>\n\nБудете получать:\n• Заявка принята\n• Заявка в работе\n• Заявка выполнена\n• Срочные уведомления\n\n/start — изменить\n/stop — отключить')
    ...
    if text in ('/start', '/role', '/роль'):
        send_msg(chat_id, '🔔 <b>LI-NING Supply — Уведомления</b>\n\nВыберите вашу роль:', start_kb())

    elif text == '/stop':
        c2 = get_db(); cur2 = c2.cursor()
        cur2.execute('UPDATE tg_subscribers SET active=FALSE WHERE chat_id=%s', (chat_id,))
        c2.commit(); cur2.close(); c2.close()
        send_tg(chat_id, '🔕 Уведомления отключены. /start — включить снова.')

    elif text == '/status':
        c2 = get_db(); cur2 = c2.cursor()
        cur2.execute('SELECT role, active FROM tg_subscribers WHERE chat_id=%s', (chat_id,))
        row = cur2.fetchone()
        cur2.close(); c2.close()
        if row:
            r = row['role']
            rname = r.replace('branch:','🏪 ') if r.startswith('branch:') else {'admin':'👔 Администратор','warehouse':'📦 Склад','regional':'📊 Региональный'}.get(r,r)
            st = '✅ Активны' if row['active'] else '🔕 Отключены'
            send_tg(chat_id, f'📊 Ваш статус:\nРоль: <b>{rname}</b>\nУведомления: {st}')
        else:
            send_tg(chat_id, '❌ Не зарегистрированы. /start')
